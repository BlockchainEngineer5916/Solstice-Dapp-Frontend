...
import { getYoutubeId, isYoutubeUrl, getFileType, removeExtension , getUuid, getCookie, fileNameFormat} from '../../../utils/Helper';
...
const UploadSolsVideo = (props) => {
    ...
    const handleChangeUploadSols = async (e) => {
        let temp = [] ;
        // console.log(getFileType(e.target.files[0].type))
        for(let i = 0 ; i < e.target.files.length ; i++) {
            ...
            temp.push({
                ...
                category : getFileType(e.target.files[i].type),
    ...
    }, [productTitle, filesForUpload, linksForUpload]) ;
                                                ...
                                                >
                                                    {
                                                        (
                                                            ...
                                                            getFileType(item.type) === 'pdf' ||
                                                            getFileType(item.type) === 'vnd.openxmlformats-officedocument.wordprocessingml.document'
                                                        ) &&
                                                        <Box className={classes.fullIconDiv} onClick={() => {
                                                            switch(getFileType(item.type)) {
                                                    ...
                                                    {
                                                        getFileType(item.type) === 'image' && <Box sx={{display : 'flex', alignItems : 'center', justifyContent : 'center', 
                                                    ...
                                                    {
                                                        getFileType(item.type) === 'pdf' && <PdfPreview
                                                    ...
                                                    {
                                                        getFileType(item.type) === 'vnd.openxmlformats-officedocument.wordprocessingml.document' && <DocxPreview
                                                            previewUrl={item.preview}
                                                            width={270}
                                                            height={400}
                                                            key={uuidv4()}
                                                    ...
                                                    {/* {
                                                        item.category === 'msword' && <DocxPreview
                                                            previewUrl={item.preview}
                                                            width={270}
                                                            height={400}
                                                            key={uuidv4()}
                                    ...
                                    filesForUpload.map((item, index) => {
                                        return <ListItem key={index} className={classes.solItemDiv}>
                                            <Box className={classes.solName}>
                                                <TextField
                                                    fullWidth
                                                    value={fileNameFormat(removeExtension(filesForUpload[index]?.name))}
                                            ...
                                            <Box className={classes.solTime}>
                                                ...
                                                {
                                                    productType === '#Ebook' && (
                                                        getFileType(item.type) === 'vnd.openxmlformats-officedocument.wordprocessingml.document' ? "Docx" :
