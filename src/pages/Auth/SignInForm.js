        ...
        if(formState.isValid) {
            ...
            if( res === 200) {
                if(
                    ...
                    if(urlFromLink) {
                        await InitAuthReducer() ;
                        navigate(urlFromLink) ;
                    }
                ...
                <Grid item xs={match800 ? 6 : 12}>
                    <Box className={classes.descriptionDiv}>
                        ...
                        <Box className={classes.welcomeDiv}>
                            Welcome
                            <br/>to { urlFromLink ? linkAccountName : ' SOLS' }
                        </Box>
                ...
                <Grid item xs={match800 ? 6 : 12}>
                    <Box className={classes.formDiv}>
                        <Grid container spacing={3}>
                            {
                                !urlFromLink ? <Grid item xs={12} className={classes.signUpDiv}>
                                    Sign In
                                </Grid> : <>
                                    <Grid item xs={12} sx={{position : 'relative', height : 'auto',display : 'flex', flexDirection : 'column !important', justifyContent : 'space-between'}}>
                                       <ProfileInfoBox />
                                    </Grid>
                                </>
                            }   
                            ...
                            <Grid item xs={12}>
                                ...
                                >
                                    {
                                        urlFromLink ? 'Connect' : 'Sign In'
                                    }
                                </Button>
                            </Grid>
                            {
                                !urlFromLink ? <Grid item xs={12}>
                                    <Button variant='contained' className={classes.buttonCss} fullWidth onClick={handleSignUp}>Create Account</Button>
                                </Grid> : <></>
                            }
                            <Grid item xs={12} >
                                <GoogleLoginButton 
                                    text={ !urlFromLink ? 'Sign In With Google' : 'Connect With Google'}
                                />
                            </Grid>
                            {
                                !urlFromLink ? <Grid item xs={12} className={classes.contentDiv}>
                                    <Box>
