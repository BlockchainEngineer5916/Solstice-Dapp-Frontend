...
const ProfileInfoBox = (props) => {
    ...
    useEffect(() => {
        if(productTypeList.length && !urlFromLink) handleCurrentProductType(productTypeList[0]) ;
    }, [productTypeList]) ;

    return (
           <>
                <Box className={classes.avatarCtrlDiv} ref={avatarCtrl} sx={{marginBottom : urlFromLink ? (width * 0.15 + (width > 370 ? 10 : 70)) + "px" : 0}}>
                    {
