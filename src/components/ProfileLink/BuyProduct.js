...
const BuyProduct = (props) => {
    ...
    const handleClickBuy = () => {
        window.open(location.origin + '/product-intro?linkId=' + productInfo.intro_link, '_blank') ;
    }
        ...
        <>
            {
                typeof productInfo !== "undefined" && <Button variant={'contained'} className={classes.buttonCss} sx={{mb : '10px', width : 'auto'}} onClick={handleClickBuy} disabled={disableButton}>
                    ...
                    {
                        productInfo?.price_type === 'bundle' && `Buy for ${productInfo?.bundle_price} ${getUnit(productInfo?.bundle_unit)}`
                    }
                </Button>
            }
        </>
