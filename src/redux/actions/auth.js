...
export const SignUpFinalUserInfo = (
    ...
    try {
        if(
            ...
            let res2 = await createCustomer( req ) ;
            ...
            await setDoc(doc(db, "Web_Users", userCredential.user.uid), {
                ...
                host_id : hostId,
                profile_message : '',
                stripe_account_id : res1.data.id,
                stripe_customer_id : res2.data.id,
    ...
    try {
        ...
        if(!userDoc.exists()) {
            let customerKey = await WriteCustomerInfo( result.user.displayName, result.user.displayName.replaceAll(' ', '').toLowerCase() , 
                result.user.email,
                "https://solsapp.com/" + result.user.displayName.replaceAll(' ', '').toLowerCase() + '.solsapp'
            ) ;
