...
export const SendAccessUrlByEmail = async (productInfo, license_key=null) =>  {
    try {
        let fromDoc = await getDoc(doc(db, "Web_Users", getCookie('_SOLSTICE_SELLER'))) ;
        let toDoc = await getDoc(doc(db, "Web_Users", getCookie('_SOLSTICE_BUYER'))) ;
        let message = `<html><body><h1>Product Name : ${productInfo?.product_name}</h1><br/>` ;

        await Promise.all(
            productInfo.sols.map(async sol => {
                if(license_key) message += sol.name + " : https://solsapp.com/sols?access_key=" + sol.access_key +
                                            "&license_key=" + license_key + " <br/>" ;
                else message += sol.name + " : https://solsapp.com/sols?access_key=" + sol.access_key + " <br/>" ;
            })
        )

        var templateParams = {
            from_email : fromDoc.data().email ,
            to_email : toDoc.data().email,
            from_name : fromDoc.data().account_name,
            to_name : toDoc.data().account_name,
            message : message
        }
        ...
        await Promise.all(
            productDocs.map(async product => {
                try {
                    ...
                    if(
                        (product_meta.data.price_type === 'bundle' || product_meta.data.price_type === 'free') 
                    ...
                    if(possible) {
                        console.log(product_meta.data.price_type, product.data().owners_ids) ;
